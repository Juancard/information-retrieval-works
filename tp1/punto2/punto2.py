...
Ejemplos:
	...
	for nombreArchivo in archivos:
		...
		for linea in archivoDic["file"]:
			...
			# Me traigo los tokens y terminos extraidos
			extraido = getRegexTokens(linea)

			# Cargo valores estadisticos
			tokensCounter += len(extraido["tokens"])
			archivoDic["token_count"] += len(extraido["tokens"])
			termCounter += len(extraido["terms"])
			archivoDic["term_count"] += len(extraido["terms"])

			# Cargo termino a vocabulario y 
			# actualiza valores de DF y CF
			resultado = lex_analyser.updateVocabulario(extraido["terms"], vocabulario, archivoDic)
			vocabulario = resultado["vocabulario"]
			archivoDic = resultado["archivo"]

			# FInalmente devuelvo la nueva linea 
			linea = extraido["final_string"]
			#------------------------------------------
